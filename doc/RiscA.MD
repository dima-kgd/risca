...
| --------------- | ----------- | ------------------------------------------------ | ----------- |
| 0               | [ALU REG REG](#alu-reg-reg) | ```unused(1)        func4(4)  Rs(4) Rd(4) opcode(3)``` | MOV,ADD,SUB,AND,OR,XOR,NOT,MUL|
...
| 4               | [BRANCH](#branch)           | ```Imm(8)           func1(1)        Rd(4) opcode(3)``` | BEQZ,BNEZ,BGTZ,BLTZ |
...
```
notused(1) | func4(4) | Rs(4) | Rd(4) | opcode(3)
```
	...
	8) SLT Rd, Rs; Rd = (Rd < Rs) ? 1 : 0 (signed)
	9) SLTU Rd, Rs; Rd = (Rd < Rs) ? 1 : 0 (unsigned)
	...
	unused
...
```
	...
	func2:
		...
		0) SHL Rd, Imm(4); Rd = Rd << Imm(0-4 bits)
		1) SHR Rd, Imm(4); Rd = Rd >> Imm(0-4 bits); Logical - fills 0 on the left
...
```
	...
	0) BEQZ Rd, Imm(8); Rd == 0 ? PC = PC + signed(Imm)<<1; in instructions (-127 .. +128 instructions)
	1) BNEZ Rd, Imm(8); Rd != 0 ? PC = PC + signed(Imm)<<1; in instructions (-127 .. +128 instructions)
